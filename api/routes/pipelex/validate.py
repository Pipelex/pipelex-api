...
async def validate_mthds(request_data: ValidateRequest) -> JSONResponse:
    ...
    Runs `validate_bundle` (sweep) + `dry_run_pipeline` (graph) in-process.
    When Temporal is enabled, the dry-run leg dispatches through the configured
    pipe-run backend like any other execution.

    NOTE: the one-round-trip Temporal offload (`wf_dry_validate` →
    `act_dry_validate`, PR #12) is temporarily reverted — the pinned pipelex
    (0.33.0, MTHDS-protocol branch) does not ship `act_dry_validate`. Restore
    the dispatch (see git history of this module) once a pipelex release
    carries both the protocol surface and the dry-validate activity.
    ...
    - **Failure (422):** RFC 7807 `application/problem+json` — same shape as
      ...
      `api.exception_handlers` unchanged. "Bundle has no `main_pipe`" is an
      API-side semantic precondition for this endpoint and is raised via
