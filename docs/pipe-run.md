...
- `pipe_output.graph_spec` (object | null): Best-effort execution graph of the run — the nodes and edges the run actually traversed. `null` when graph tracing was off for the run. `pipe_output.graph_assembly_error` (string | null) is non-null when graph assembly failed, which is what distinguishes a failed assembly from tracing having been off.
- `pipe_output.pipe_io_artifacts` (object | null): The I/O artifacts describing the graph's data, carried beside `graph_spec` so that wherever the graph travels its description travels with it. It groups the same artifacts `/validate` produces — `pipe_io_contracts`, `input_form` and `output_form` — each keyed by namespaced `pipe_ref`, over the pipes of the library the run executed against. Holding it with the graph is what lets a viewer show a data node's value rather than only its shape. `null` when the run assembled no graph; `pipe_output.pipe_io_artifacts_error` (string | null) is non-null when the build failed. See [Pipe Validate](pipe-validate.md) for the artifacts' own field reference.
