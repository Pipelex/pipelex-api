...
## [Unreleased]
### Changed — `/validate` rewired through the protocol runner and turned into a 200-diagnostic endpoint (MTHDS Protocol surface alignment, Phase 2)

`/validate` (like `/models`) now routes through `ApiRunner` (extending `PipelexMTHDSProtocol`) exactly like `/execute` and `/start` already did — the runner owns backend selection (in-process vs ONE dispatched Temporal activity), the runtime owns the canonical artifact shapes, and the route only adds wire extras. Hosted and local runners answer with identical artifacts. Requires an unreleased pipelex (> 0.33.0) carrying the canonical `PipelexValidationReport` / `build_validation_report` / `DryValidateResult` surfaces plus the structured-error contract.

- **An invalid bundle is a produced verdict (200), not a transport failure (422).** The route catches `ValidateBundleError` (direct and Temporal-recovered) and returns a **200 `InvalidReport`** (`is_valid: false`) instead of a 422 `problem+json`. The 200 body is a discriminated union keyed on `is_valid` — `ValidReport` (`is_valid: true`, structural artifacts) or `InvalidReport` (`is_valid: false`, `validation_errors[]`). The redundant `success` wire extra is retired in favor of `is_valid`. Non-2xx is reserved for *no-verdict* conditions only: a malformed request body, an `mthds_sources` length mismatch (request-shape 422), auth (401/403), or a server fault (5xx). This subsumes the old `main_pipe` precondition — a bundle that declares no `main_pipe` now validates with **200** and `graph_spec: null` (was a 422), on both backends.
- **`ValidReport` is the canonical validation report.** `pipelex_bundle_blueprint` → **`bundle_blueprint`** (primary blueprint — first file declaring `main_pipe`, else first). `pipe_io_contracts` is keyed by **namespaced `pipe_ref`** (`domain.code`, was bare pipe code) with typed entries, and each entry names its input/output concept **`concept_ref`** (was `concept_code`, now always the namespaced `concept.concept_ref`, reading consistently next to the map's `pipe_ref` key). New fields: `validated_pipes` (per-pipe sweep outcomes, `{pipe_ref, status}`), `pending_signatures` (refs of unimplemented `PipeSignature` declarations) and `is_runnable` — the runnability verdict finally crosses the HTTP surface. The OpenAPI artifact renames the `PipeInputContract` / `PipeOutputContract` property accordingly (schema *names* unchanged). `BuildConceptResponse.concept_code` on `/v1/build/concept` is unaffected — it accepts a bare or qualified code, so `_code` stays correct there.
- **`InvalidReport.validation_errors[]` is a non-empty structured list.** Each item carries `category`, `message`, the pipelex classification (`error_type`, `pipe_code`, `concept_code`, `domain_code`), and the field/source locators (`source`, `field_path`, `field_name`, plus `variable_names` / `missing_concept_code` / `declared_concepts` where applicable). The structured-info invariant guarantees the list is non-empty on every invalid verdict that reaches the wire. The items are builder-identical to the agent CLI's `validation_errors[]` (one shared pipelex builder, two surfaces).
- **New optional `mthds_sources` on `POST /v1/validate`.** A per-file source list, parallel to `mthds_contents`. When provided, each source is threaded onto the corresponding `blueprint.source`, so the structured errors — and the success report's `bundle_blueprint.source` — name the owning file. This is the mechanism a multi-file editor client uses to map cross-file diagnostics back to their source. Additive: sourceless callers are unchanged. A length mismatch against `mthds_contents` is a request-shape 422 (caught at request validation, before the runtime would treat it as an internal error).
- **Temporal mode is pure dispatch + map.** The worker computes everything library-dependent (`pipe_io_contracts`, `pending_signatures`, `graph_spec`, status map, structured errors) inside its single library load and ships it on the activity result; the API side no longer re-acquires a library to build response artifacts. The OpenAPI artifact was regenerated for the union response.

### Changed — `/models` returns the protocol `ModelDeck`

- `/models` now routes through `ApiRunner` and returns the protocol `ModelDeck`. The flat `models` list (`{name, type}` entries — the old raw per-category payload parsed into a silently EMPTY deck in the `mthds` SDK) plus this implementation's `aliases`/`waterfalls` routing extensions, keyed by model category (the same alias name exists in several categories pointing at different models — a flat map would silently drop entries). The old raw keys (`presets` keyed by category, `success`) are **dropped** — no verified consumer reads them. The `type` query param is now a **single** value matching the protocol arity: repeated `?type=` values are a 422 `ValidationError` (the old multi-category extension is dropped), unknown categories — including an explicit empty `?type=` — remain a 422 `InvalidModelCategory`.

### Changed — Protocol version constant comes from the SDK

- `/version` and the OpenAPI metadata import `PROTOCOL_VERSION` from `mthds.protocol.protocol`; the `pipelex.pipeline.runner.MTHDS_PROTOCOL_VERSION` alias is deleted upstream.

### Changed — track pipelex validation-errors source-threading rev

- Bumped the pinned `pipelex` git rev to carry the structured-error contract (`ValidationErrorItem`, `ErrorReport.validation_errors`) and the `validate_bundles_in_process` in-process orchestrator with per-content `mthds_sources` source threading. `ApiRunner.validate` calls that orchestrator directly on the in-process backend (threading `mthds_sources`) and threads sources through `DryValidateArg` + the blueprint parse on the Temporal backend.
