...
- **`/v1/resolve` and `/v1/codegen` are listed as MTHDS Protocol routes** in the API description and the docs — they carry `x-mthds-protocol: true` (resolution and type-projection are protocol capabilities), so the "five protocol routes" framing was stale.
