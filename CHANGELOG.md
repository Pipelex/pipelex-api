...
- **`pipelex` resolves from a git source, temporarily**: the structured-log seam above is in no published release, so `pyproject.toml` points `pipelex` at a branch of the runtime repository through `[tool.uv.sources]` instead of the exact PyPI pin. Every install path this repository has goes through `uv` — `make install`, CI's `uv lock --check`, and the Docker build's `uv sync --frozen` — and `uv` honours that entry, including through its pip interface. A plain `pip install .` does not read it, and because the branch build declares a version number the published releases already carry, no version specifier can select it; such an install therefore resolves a *published* `pipelex`, which has none of this. The runner now refuses to start in that case with a message naming the cause, checked at import before anything reads a configuration — rather than failing on its own `[runtime.log] sink` key with an error that does not say why, or raising on the first error it tries to log. It collapses back to an exact published version before the next release.
