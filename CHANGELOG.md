...
- **The model roster this image serves does not move with the pin**: `pipelex` 0.61.0 adds the GPT-5.6 series, `gpt-6-astra`, Claude Opus 5 and the Claude Fable 5 and 5.1 handles to the `openai`, `azure_openai` and `anthropic` templates in its own bundled kit, and retires the older GPT generations from them — but this server never reads that kit. It ships its own `.pipelex/inference/`, which the Dockerfile relocates to `/root/.pipelex` so a user mount is not shadowed, and pipelex resolves backends and deck from that one directory with no fallback to the kit inside the wheel. The vendored rosters are untouched by this bump, so `GET /v1/models` answers exactly what it answered under 0.60.0, no handle is retired, and nothing here is breaking. Picking the new handles up is a separate change that re-syncs the vendored tree; on the default deployment, where only `pipelex_gateway` is enabled, the live roster comes from the remote config artifact and moves when that is redeployed rather than when this pin does.
