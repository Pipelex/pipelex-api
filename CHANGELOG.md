...
- **Pinned `pipelex` 0.59.0**: up from `==0.58.0`, exactly, so a run no longer renders every traced input and output to Rich text and HTML on the execution path, which on the hosted runner happened on the Temporal worker's workflow thread. The release also stops probing remote inputs before a run starts, fixes the "Output of pipe" panels hanging a run in `poor` pretty-print mode on a narrow headless console, and escapes every `<` in the graph and stuff viewer pages. It adds `pretty_print_mode` (`rich`, `poor` or `silent`) under `[runtime.log]`, which this server leaves at its `rich` default.
