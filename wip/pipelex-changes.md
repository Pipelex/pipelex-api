...
| 10 | `EnvVarNotFoundError` → `error_domain = CONFIG` | 7 | `system/exceptions.py` (moved from `system/environment.py` in Phase 6) | Decisions taken (2026-05-28), implementation pending on `feature/API-readiness-4` | — |
| 11 | `parse_concept_spec` shape-validation of `structure` | 7 | `builder/operations/concept_ops.py` | Decisions taken (2026-05-28), implementation pending on `feature/API-readiness-4` | — |
| 12 | `LocalStorageProvider` wrap `OSError` as new `StorageLocalError` | 7 | `tools/storage/local_storage_provider.py` + `tools/storage/exceptions.py` | Decisions taken (2026-05-28), implementation pending on `feature/API-readiness-4` — new `StorageLocalError(StorageError)` class to parallel `StorageS3Error` / `StorageGcpError` | — |
| 13 | `S3StorageProvider` catch full `BotoCoreError` hierarchy | 7 | `tools/storage/s3_storage_provider.py` | Decisions taken (2026-05-28), implementation pending on `feature/API-readiness-4` — keeps wrapping as existing `StorageS3Error` (no new sub-class) | — |
| 14 | `ErrorDomain.is_input` (and siblings) `@property` helpers | 7 | `base_exceptions.py` | Decisions taken (2026-05-28), implementation pending on `feature/API-readiness-4` | — |
| 15 | `kajson` wrap bare `KeyError`/`AttributeError`/`TypeError` in `KajsonDecoderError` | 7 | `kajson/json_decoder.py` (note: kajson repo, not pipelex) | Not started — discovered in pipelex-api Phase 3 review (Q10); out of scope for `feature/API-readiness-4` (target repo is kajson, not pipelex) | — |

Stages 1–4 (#1–#7) are landed — the `pipelex-api` plan is unblocked: Phase 0 consumes #1+#2, Phase 1 consumes #4 (`to_dict(disclosure_mode=)`) and #6 (`to_problem_document`), Phase 4 consumes #5. Only #8 (future, no consumer yet), #9 (webhook signing — separate track), and #10–#15 (post-Phase-3 audit findings, non-blocking) remain. Decisions taken on #10–#14 on 2026-05-28; implementation pending on the pipelex-side branch `feature/API-readiness-4` — see `_for_api/TODOS.md` for the per-item plan. #15 is out of scope for that branch (target repo is kajson, not pipelex).
