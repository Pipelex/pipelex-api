...
- Body-derived observability fields (`pipe_code`, `pipeline_run_id`) now ride every error log — `_parse_request` writes both to `request.state` after the parse succeeds and the three log paths read them via `_pipe_code_of` / `_pipeline_run_id_of`. Together with the `user_id` slice that landed via Phase 3 review Q9, this discharges Checkpoint B reconciliation #4.
...
- **Q9:** `user_id` rides every error log when an authenticated user is present (`request.state.user.user_id`, set by `_set_request_user`). `pipe_code` / `pipeline_run_id` now ride every error log too — `_parse_request` writes both to `request.state` after the parse succeeds, and `_pipe_code_of` / `_pipeline_run_id_of` getters in `api/exception_handlers.py` feed them into the three log paths (`_log_error_report`, `_log_api_authored_error`, `handle_unexpected_error`). Discharges the last piece of Checkpoint B reconciliation #4.
- **Q10:** `_decode_body`'s catch tuple is `(UnicodeDecodeError, ValueError, KajsonDecoderError, KeyError, AttributeError, TypeError, RecursionError)` — covers every documented kajson decode failure plus the bare three (`KeyError`/`AttributeError`/`TypeError`) kajson leaks on crafted `__class__` / `__module__` markers AND `RecursionError` from deeply-nested JSON arrays exhausting the parser's recursion budget. The `try` scope is one line (`kajson.loads(...)`), so the Q4 rule against broadly catching `AttributeError`/`TypeError` doesn't apply. Upstream wrap-the-bare-three filed as `pipelex-changes.md` #15 (target repo: kajson, not pipelex). Security concern (kajson imports arbitrary modules from caller-controlled body) tracked separately at `wip/security/kajson-untrusted-deserialization.md`.
...
- **JSON log sink.** **Upstream-blocked.** Native structured emission requires pipelex's `log` API to accept structured fields (today it's string-shaped end-to-end — `log.info(content: str | Any, ...)` with no `extra=` / `**fields`). The structured-logging refactor is on its own future branch upstream — see `_for_api/wip/structured-logging/kickoff.md` (open question 1 is whether the formatter layer preserves `LogRecord.extra` at all). The current logfmt rendering in `emit_error_log` (with `_logfmt_value` injection-safe escaping) is a clean migration target once the upstream surface lands.
...
Items that belong in `pipelex/` rather than `pipelex-api/`, surfaced during this PR's audits. Track here so they don't get lost; land via separate pipelex PRs. **The pipelex-side branch is `feature/API-readiness-4` and its plan lives at `_for_api/TODOS.md`** — read that for the per-item context, design choices, and sequencing. The four open questions on that branch were resolved on 2026-05-28 — the annotations below record the decisions so the API-side migration step for each item is pre-documented.

- **Stage 7 items #10–#14** in `wip/pipelex-changes.md`, all on `feature/API-readiness-4`. Implementation pending; decisions taken on each:
  - #10 `EnvVarNotFoundError` → `error_domain = CONFIG`. Once landed, the API's `test_env_var_not_found_error_is_domainless_500` (in `tests/unit/test_exception_handlers.py`) flips to assert `error_domain == "config"` and remove the "domainless" branch.
  - #11 `parse_concept_spec` shape validation. Once landed, the API's `/build/concept` route gains a narrow typed catch (today it cannot safely catch `AttributeError`/`TypeError`, because they're also the types a real programming bug would raise).
  - #12 `LocalStorageProvider` `OSError` wrap. **Decision:** new `StorageLocalError(StorageError)` class (paralleling `StorageS3Error` / `StorageGcpError`). Once landed, any narrow catch the API adds for local-storage failures should target `StorageLocalError` specifically.
  - #13 `S3StorageProvider` widen to full `BotoCoreError` hierarchy. **Decision:** keep wrapping as the existing `StorageS3Error` (no new sub-class). **No API-side change** — the existing storage error handling already classifies `StorageS3Error` correctly.
  - #14 `ErrorDomain.is_input` (and siblings) `@property` helpers. Once landed, the two call sites in `api/exception_handlers.py` deliberately left on `report.error_domain == ErrorDomain.INPUT` (per `Q9` in the Phase 3 review — see `archive-todos-api-readiness-2.md`) become a one-commit swap to `report.error_domain.is_input` / `error_domain.is_input`. That follow-up pointer is still accurate and no further documentation is needed here.

  Item #15 (kajson crafted-marker exceptions) is **out of scope for `feature/API-readiness-4`** — target repo is `kajson`, not `pipelex`. Belongs on a separate kajson PR. The API has a workaround in place (`_decode_body`'s widened catch tuple at `pipeline.py:156`).

- **Webhook-delivery SSRF DNS recheck.** `api/schemas/models.py::_is_disallowed_host` only blocks literal private/loopback/metadata IPs at request time — a callback URL like `https://attacker.example/cb` passes validation while its DNS record can resolve to `169.254.169.254` / `127.0.0.0/8` / `10.0.0.0/8` when the worker later fires the webhook. The fix belongs at delivery time in `pipelex/pipe_run/delivery_executor.py`'s webhook HTTP client: a custom resolver (or `httpx` event hook) that re-checks the resolved IP against the same private-host rule and aborts on a private destination, plus optionally an egress allowlist / proxy at the deploy layer. Keep the literal-host check in the API as a cheap first line of defense. **Decision (2026-05-28):** the disallowed-host rule moves into a new `pipelex/tools/network/` package as `pipelex.tools.network.is_disallowed_host(...)` (greenfield — no `network` module exists today under `pipelex/tools/`); the API-side `_is_disallowed_host` then becomes a one-line re-export. **API-side migration step (post-merge):** delete the local CIDR rule in `api/schemas/models.py::_is_disallowed_host` and replace with `from pipelex.tools.network import is_disallowed_host` so the rule lives in one place.

- **Structured `event=webhook_delivery` / `event=webhook_failure` logging — DEFERRED.** The narrow event-name pass on `pipelex/pipe_run/delivery_executor.py` (around lines 239 / 243 / 280 / 282 / 285) was **re-targeted from `feature/API-readiness-4` onto the future `refactor/structured-logging` branch** on 2026-05-28. Rationale: those lines are explicit deletion targets in the structured-logging refactor (the `request_id_suffix` interpolation pattern and the manual `request_id` kwarg threading from commits `ceb018b5` / `07f9cce9`). Doing a narrow event-name pass now means touching them twice. The kickoff doc at `_for_api/wip/structured-logging/kickoff.md` "What good looks like" now folds `event=webhook_delivery` / `event=webhook_failure` (and `event=storage_delivery` / `event=storage_failure` for symmetry) into the destination shape. The receiver-side consistency T6 test (`tests/unit/test_webhook_recovery.py`) pins error-rendering, not event names — the API is not blocked. The plumbing landed upstream in Phase A1 (`ceb018b5` / `07f9cce9` thread `request_id` through `DeliveryExecutor`); only the event-name emission moves to the structured-logging branch.
